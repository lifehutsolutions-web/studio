...
export async function onRequestPost(context: { request: Request; env: any }) {
  ...
  try {
    const { orderId, paymentId, signature, productId } = await request.json();

    // 1. Fetch product from Supabase to verify it exists and get details
    ...
    if (orderId.startsWith("order_mock_")) {
      ...
      isVerified = await verifyHmacSha256(RAZORPAY_KEY_SECRET, orderId + "|" + paymentId, signature);
    }

    if (isVerified) {
      // Generate secure temporary download token valid for 24 hours
      // Format: productId.filename.expiryTimestamp.signature
